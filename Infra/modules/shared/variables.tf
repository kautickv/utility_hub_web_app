...
variable "terraform_backend_key" {
  description = "Key path in the S3 bucket for Terraform state files"
  type        = string
  default     = "terraformStateFiles"
}

variable "terraform_backend_region" {
  description = "Region for the S3 bucket used for Terraform backend"
  type        = string
  default     = "us-east-1"
}
